...
class PHP_Email_Form {
  ...
  public function send() {
    ...
    if ( version_compare(PHP_VERSION, '5.5.0') >= 0 ) {
      ...
      $sendemail = new PHPMailer_Legacy(true);
    }

    // Check and set SMTP
    if( is_array( $this->smtp) ) {
      $sendemail->isSMTP();
      $sendemail->Host = $this->smtp['host'];
      $sendemail->SMTPAuth = true;
      $sendemail->Username = $this->smtp['username'];
      $sendemail->Password = $this->smtp['password'];
      $sendemail->Port = $this->smtp['port'];
      $sendemail->SMTPSecure = $this->smtp['encryption'];

      $this->mailer = $this->smtp['username'];
    }

    // Headers
    $sendemail->CharSet = $this->charset;
    $sendemail->ContentType = $this->content_type;

    // Recipients
    $sendemail->setFrom($this->mailer, $from_name);
    $sendemail->addAddress( $to );
    $sendemail->addReplyTo($from_email, $from_name);
    ...
    if( $sendemail->send() ) {
      ...
      return 'Mailer Error: ' . $sendemail->ErrorInfo;
    }
  }
}
